...
const textSet = [
    { text: "BITCOIN", sphereRadius: 140, sphereSpace: 80, unitTime: 100, time: 1000 },
    { text: "ETHEREUM", sphereRadius: 120, sphereSpace: 70, unitTime: 120, time: 4000 },
    { text: "TRONIX", sphereRadius: 120, sphereSpace: 70, unitTime: 50, time: 2000 },
    { text: "ONTOLOGY", sphereRadius: 120, sphereSpace: 70, unitTime: 100, time: 4000 },
...
class Vertex3D {
    constructor(config) {
        this.affineIn = {
            vertex: config.vertex || { x: 0, y: 0, z: 0 },
            size: config.vertex || { x: 1, y: 1, z: 1 },
            rotate: config.rotate || { x: 0, y: 0, z: 0 },
...
class Sphere {
    constructor(args) {
        ...
        this.targetCenter = args.center;
        this.radius = 0;
        this.targetRadius = args.radius;
        this.degree = [];
...
function rectangleToPolar(x, y, z) {
    let xD = x || 0.001;
    let yD = y || 0.001;
    let zD = z || 0.001;
...
function setup() {
    for (let i = 0; i < sphereNum; i++) s.push(new Sphere({
        radius: 100,
        particleNum: 250,
...
function textChanger(text, sphereRadius, sphereSpace, unitTime) {
    var changeIncrement = 0;
    var charNum = text.length;
    var center = [];

    for (var i = 0; i < charNum; i++) {
        center.push({ x: sphereSpace * i - sphereSpace * (charNum - 1) / 2, y: 0, z: 0 });
    }
